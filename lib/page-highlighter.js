...
function initHighlighter(highlighterType) {
...
export class PageHighlightManager {
  ...
  renderCurrentRange() {
    ...
    let lineRect = currentLine.rect;
    let lineRectWithOffsets = new DOMRect(
      lineRect.x,
      lineRect.y + (currentLine.topOffset || 0),
      lineRect.width,
      lineRect.height + (currentLine.bottomOffset || 0),
    );

    this.highlightLineAndWord({
      rangeRect: this._range.getBoundingClientRect(),
      lineRect: lineRectWithOffsets,
  ...
  highlightLineAndWord({
    ...
    const wordRect = new DOMRect(
      rangeRect.x - lineRect.x,
      fullWidthLineRect.y,
      rangeRect.width,
      Math.max(rangeRect.height, lineRect.height),
    );
    ...
    requestAnimationFrame(() => {
      ...
      this.highlighterElem.renderAreas(fullWidthLineRect, wordRect);
    });
  }
}
